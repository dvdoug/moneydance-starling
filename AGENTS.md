...
- **Pending set-reconcile our `starling:pending:` register rows**, including after the user confirmed the blue dot. Unique pending→posted match (exact amount, merchant, date within 7 days) updates that parent in place (posted FITID, clear `starling.pending`, drop `[PENDING] `). If it leaves Starling pending **without** a unique match (amount changed, ambiguous, vanished): `deleteItem` that parent even if confirmed, then add posted as a download when we have one. Never delete reminder/typed rows or posted `starling:` FITIDs. Unconfirmed pending **Description** gets `[PENDING] ` **after** `showDownloadedTxns`. Never put that prefix on `OnlineTxn.setName` / `ol.orig-payee`.
